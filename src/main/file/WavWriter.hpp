    ...
    private:
        ...
        static bool is16BitPcmWavFile(std::istream &is)
        {
            ...
            while (is.read(chunkId, 4) &&
                   is.read(reinterpret_cast<char *>(&chunkSize), 4))
            {
                if (std::memcmp(chunkId, "fmt ", 4) == 0)
                {
                    std::vector<char> fmtData(chunkSize);
                    if (!is.read(fmtData.data(), chunkSize))
                    {
                        return false;
                    }
                    uint16_t audioFormat =
                        *reinterpret_cast<uint16_t *>(fmtData.data());
                    uint16_t bitsPerSample =
                        *reinterpret_cast<uint16_t *>(fmtData.data() + 14);
        ...
        static std::ofstream
        openOutputFileStream(const std::filesystem::path &p)
        {
        ...
        static void writeDataChunk(const cupuacu::State *state,
                                   std::istream &is, std::ostream &os)
        {
            ...
            // Write PCM16 samples
            os.write(reinterpret_cast<const char *>(interleaved.data()),
                     dataSize);
        }
        ...
        static bool isCopyBinaryIdentical(const std::filesystem::path &original,
                                          const std::filesystem::path &copy)
        {
            ...
            std::streampos dataStart1 = findDataChunkStartOffset(ifs1) +
                                        std::streampos(8); // skip "data"+size
            std::streampos dataEnd1 = findDataChunkEndOffset(ifs1);

            std::streampos dataStart2 =
                findDataChunkStartOffset(ifs2) + std::streampos(8);
