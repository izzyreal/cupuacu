    ...
    class WaveformCache {
        ...
        {}
            ...
            void invalidateSamples(int64_t startSample, int64_t endSample)
            {
                ...
                if (levels.empty() || numSamples <= 0) return;
            ...
            int getLevelIndex(double samplesPerPixel) const
            {
                ...
                double eff = std::max(1.0, samplesPerPixel / (double)BASE_BLOCK_SIZE);
            ...
            void applyInsert(int64_t posSample, int64_t countSamples)
            {
                ...
                dirtyFromBlock = std::min<int64_t>(dirtyFromBlock, posSample / BASE_BLOCK_SIZE);
                dirtyToBlock = level0Size() - 1;
            }
            ...
            void applyErase(int64_t startSample, int64_t endSample)
            {
                ...
                if (levels.empty())
                {
                    numSamples = std::max<int64_t>(0, numSamples - (endSample - startSample));
                    buildStorage();
                    markAllDirty();
                    return;
                }
            ...
            int levelsCount() const { return (int)levels.size(); }

            void rebuildDirty(const float* samples)
            {
                if (levels.empty() || numSamples <= 0) { dirtyFromBlock = INT64_MAX; dirtyToBlock = -1; return; }
                if (dirtyToBlock < dirtyFromBlock) return;

                int64_t max0 = level0Size() - 1;
                if (max0 < 0) { dirtyFromBlock = INT64_MAX; dirtyToBlock = -1; return; }
                ...
                for (int64_t blk = from0; blk <= to0; ++blk)
                {
                    ...
                    int64_t s1 = std::min<int64_t>(s0 + BASE_BLOCK_SIZE, numSamples);
                    if (s0 >= s1) { levels[0][blk] = { 0.0f, 0.0f }; continue; }
        ...
        private:
            ...
            int64_t levelSize(int level) const
            {
                ...
                for (int l = 1; l <= level; ++l) sz = (sz + 1) / 2;
                return sz;
            }
