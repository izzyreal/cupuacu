...
static std::vector<float> splineInterpolate(const std::vector<int16_t>::const_iterator begin,
                                            const std::vector<int16_t>::const_iterator end,
                                            int newNumberOfDataPoints)
{
    int n = static_cast<int>(std::distance(begin, end)) - 1; // number of intervals
    if (n < 1 || newNumberOfDataPoints < 2) return {};
    ...
    // Compute the spline coefficients by solving tridiagonal system for second derivatives (M)
    ...
    for (int i = 0; i < newNumberOfDataPoints; ++i)
    {
        ...
        double val = a[interval] + b[interval]*t + c[interval]*t*t + d[interval]*t*t*t;
