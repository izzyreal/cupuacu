...
public:
    ...
    virtual void resize(int64_t numChannels, int64_t numFrames)
    {
        ...
        for (auto &ch : channels) ch.resize(numFrames);
    }

    virtual void setSample(int64_t channel, int64_t frame, float value, const bool shouldMarkDirty = true)
    {
    ...
    virtual void insertFrames(int64_t frameIndex, int64_t numFrames)
    {
        ...
        for (auto &ch : channels) ch.insert(ch.begin() + frameIndex, numFrames, 0.0f);
    }

    virtual void removeFrames(int64_t frameIndex, int64_t numFrames)
    {
        int64_t chCount = channels.size();
        for (auto &ch : channels) ch.erase(ch.begin() + frameIndex, ch.begin() + frameIndex + numFrames);
    }
