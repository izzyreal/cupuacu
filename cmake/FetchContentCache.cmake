...
function(fetchcontent_try_prepare_git_source dep_name source_dir repository git_tag)
  ...
  if(_needs_init)
    ...
    fetchcontent_run_git(_init_result _init_output _init_error -C "${source_dir}" init)
    if(NOT _init_result EQUAL 0)
      message(FATAL_ERROR "FetchContent ${dep_name}: git init failed in ${source_dir}: ${_init_error}")
    endif()
    fetchcontent_run_git(_remote_remove_result _remote_remove_output _remote_remove_error -C "${source_dir}" remote remove origin)
    fetchcontent_run_git(_remote_add_result _remote_add_output _remote_add_error -C "${source_dir}" remote add origin "${repository}")
    if(NOT _remote_add_result EQUAL 0)
      fetchcontent_run_git(_remote_set_result _remote_set_output _remote_set_error -C "${source_dir}" remote set-url origin "${repository}")
      if(NOT _remote_set_result EQUAL 0)
        message(FATAL_ERROR "FetchContent ${dep_name}: failed to configure origin in ${source_dir}: ${_remote_add_error}${_remote_set_error}")
      endif()
    endif()
    fetchcontent_run_git(_fetch_result _fetch_output _fetch_error -C "${source_dir}" fetch --depth 1 origin "${git_tag}")
    if(NOT _fetch_result EQUAL 0)
...
function(fetchcontent_declare_cached_git dep_name cache_subdir default_path repository git_ref)
  ...
  elseif(IS_DIRECTORY "${_source_dir}")
    ...
    elseif(EXISTS "${_source_dir}/.git")
      message(STATUS "FetchContent ${dep_name}: repairing incomplete source ${_source_dir}")
      fetchcontent_try_prepare_git_source("${dep_name}" "${_source_dir}" "${repository}" "${_git_tag}")
      set(${_dep_override_var} "${_source_dir}" PARENT_SCOPE)
