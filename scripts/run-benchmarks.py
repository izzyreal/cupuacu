...
def summarize(rows):
    ...
    for key, values in sorted(groups.items(), key=lambda pair: str(pair[0])):
        ...
        for row in valid:
            ...
            for group in ("event_latency", "event_loop", "waveform_queries", "navigation_dispatch", "navigation_during_work", "bounded_storage", "effect_command", "coordination", "workflow", "decoded_cache", "clipboard_paste", "new_document", "recording", "m4a_metadata", "legacy_recovery", "audio_memory", "peak_paging", "index_paging"):
