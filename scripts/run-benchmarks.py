...
def summarize(rows):
    ...
    for key, values in sorted(groups.items(), key=lambda pair: str(pair[0])):
        ...
        for row in valid:
            ...
            for group in ("event_latency", "event_loop", "waveform_queries", "navigation_dispatch", "navigation_during_work", "bounded_storage", "effect_command", "coordination", "workflow", "decoded_cache", "clipboard_paste", "new_document", "recording", "m4a_metadata", "legacy_recovery", "audio_memory", "peak_paging", "index_paging", "working_memory", "edit_metadata", "legacy_metadata"):
...
def cases(profile, sizes, suites):
    for suite in suites:
        ...
        if profile != "quick":
            for mib in sizes:
                for scenario in (("open_uncached", "open_cached", "open_decoded_cached", "large_file_workflow") if suite == "core" else ("open_uncached", "open_cached")):
                    for mode in (["timing", "diagnostic"] if suite == "core" else ["timing"]):
                        yield {"suite": suite, "mode": mode, "scenario": scenario, "frames": int(mib * 1048576 / 8),
